...
pub struct SrcAccessChanged<'a>
{
    ...
    // the current archetype and its column/changed offsets, resolved once when we step onto
...
impl<'a> SrcAccessChanged<'a>
{
    ...
    /// Reads the changed tick of the row just handed out by `next`/`next_mut`
    #[inline]
    ...
    #[inline]
    pub(crate) fn set_changed_tick_at(&mut self, row: usize, tick: ChangedTick)
    {
        unsafe { write_changed_tick(self.current_changed_ptr, row, tick) }
    }
}
