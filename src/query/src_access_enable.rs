...
pub struct SrcAccessEnable<'a, const V: bool>
{
    ...
    // the current archetype and its column/enable offsets, resolved once when we step onto
...
impl<'a> TQuerySrcAccess<'a> for SrcAccessEnable<'a, true>
{
...
impl<'a, const V: bool> SrcAccessEnable<'a, V>
{
    ...
    /// Reads the enable bit of the row just handed out by `next`/`next_mut` (fast path: no
    /// `component_col_descriptors` lookup, the pointer was resolved once for the whole chunk)
    #[inline]
    pub(crate) fn is_enabled_at(&self, row: usize) -> bool
    {
        unsafe { read_bit(self.current_enable_ptr, row) }
    }
    #[inline]
    pub(crate) fn set_enabled_at(&mut self, row: usize, value: bool)
    {
        unsafe { write_bit(self.current_enable_ptr, row, value) }
    }
}
