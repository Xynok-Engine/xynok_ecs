...
/// extra element. Every count in [`ENTITY_COUNTS`] divides evenly by [`FRAGMENTED_ARCHETYPES`], so
/// in practice the buckets always come out equal, but this stays correct if that ever changes.
...
impl ArchetypeLayout
{
    ...
    /// This is also how the report matches a criterion result back to a scenario, so changing it
    /// invalidates any criterion data already on disk.
    ...
    /// The same thing written for a human reading a table.
    ...
    {
        Self::ALL.into_iter().find(|layout| layout.slug() == slug)
    }
...
/// Entity counts for the query benchmark. 1k fits in L2 on most machines, 100k does not, so the
/// pair says something about cache behaviour and not only about instruction count.
pub const ENTITY_COUNTS: [usize; 3] = [1_000, 10_000, 100_000];

/// Query arities under test. Every extra component is another column to walk in lockstep, so the
/// slope across these says how the query machinery scales with arity rather than with size.
pub const COMPONENT_COUNTS: [u8; 3] = [1, 2, 3];
...
/// Parses a [`count_label`] back into the number it came from.
pub fn count_from_label(label: &str) -> Option<usize>
{
    if let Some(rest) = label.strip_suffix('M')
    {
        rest.parse::<usize>().ok().map(|n| n * 1_000_000)
    }
    else if let Some(rest) = label.strip_suffix('k')
    {
        rest.parse::<usize>().ok().map(|n| n * 1_000)
    }
    else
    {
        label.parse().ok()
...
/// One competitor in the query benchmark.
///
/// `setup` and `prepare_query` are allowed to allocate and are never timed. `run_query_once` is the
/// only part that is: it must not allocate, and the report binary checks exactly that.
pub trait QueryWorkload
{
    type Storage;
    type PreparedQuery;

    /// Library name, as it appears in benchmark ids. Keep it free of spaces and `/`, because
    /// criterion turns it into a directory name.
    const NAME: &'static str;
    /// How the same library is spelled in prose and in the report tables.
