...
{
    // A `Query` borrows the world mutably, so it cannot be kept between runs. Each run builds its
    // own, and after the first run that is only a lookup in the query registry.
    type PreparedQuery = ();
    type Storage = World;
    ...
    fn prepare_query(_storage: &mut World) -> Self::PreparedQuery {}
