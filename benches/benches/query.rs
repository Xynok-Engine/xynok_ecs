...
//! ```
//!
//! Criterion owns every part of the measurement here. It picks the iteration counts, runs the
//! warm-up, collects the samples, fits a slope through them and bootstraps the confidence
//! intervals, flags outliers, and compares the run against the previous one on disk. That is the
//! reason this file is as short as it is: all it has to do is describe the work.
//!
//! One benchmark is one full pass over every matching entity. The storage and the query are built
//! once, before the timed region, and reused across every sample: rebuilding a 100k entity world
//! per iteration would drown the thing being measured, and none of these libraries asks a user to.
//!
//! Benchmark ids read `query/<layout>/<n>_components/<library>/<entity count>`, so the criterion
//! report puts the three libraries on one chart per scenario, and `src/bin/report.rs` can take that
//! id apart again to rebuild the comparison table. Throughput is set to the entity count, which
//! makes `elem/s` comparable across entity counts as well as across libraries.
//!
//! What each axis is asking:
//!
//! - **layout**: `1_archetype` is the friendly case, one contiguous run of entities. `5_archetypes`
//!   splits the same entities across five archetypes that a query has to fan out over, which is
//!   what any real world looks like after a few frames of adding components.
//! - **components**: 1, 2 and 3. Every extra component is another column to walk in lockstep, so
//!   the slope across these three says how the query machinery scales with arity, not with size.
//! - **entity count**: 1k fits comfortably in cache, 100k does not.
//!
//! Passing order is deliberate: for each scenario the three libraries run back to back, so a
//! machine that heats up or drifts in clock speed over a long run drags all three the same way
//! instead of systematically penalising whichever one was registered last.
...
/// Criterion's default is 3s of measurement per benchmark. There are 54 of them here (2 layouts x
/// 3 arities x 3 sizes x 3 libraries), so the defaults would run for minutes on end without the
/// extra samples telling us anything the shorter window does not.
